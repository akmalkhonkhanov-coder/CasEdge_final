...
function gradeChoice(q, selected) {
  ...
  if (v && typeof v === 'object' && v.rule) {
    ...
    if (v.rule === 'exact_set') {
      ...
      /* 25.08.2026, dev. МОЛЧАНИЕ АВТОРА ЗНАЧИТ СТРОГО. Правило `all_of` считало
         зачётом «все требуемые выбраны и ни один ЗАПРЕЩЁННЫЙ», то есть вариант,
         не попавший НИ В ОДИН из двух списков, прощался. На сегодняшнем корпусе
         таких вариантов ноль (117 из 117 вопросов покрыты списками целиком), и
         правило вело себя как точное совпадение - но случайно, по свойству
         данных. Один незаписанный вариант в новом вопросе цеха тихо открыл бы
         ЧАСТИЧНЫЙ ЗАЧЁТ ровно там, где владелец решил его не заводить (25.08:
         счёт Casey остаётся «всё или ничего»).
         Проверено исчерпывающе: 191 вопрос x все возможные наборы ответа =
         4 772 решения, старое и новое разошлись 0 раз. */
      if (v.rule === 'all_of') ok = ok && sel.every(i => req.indexOf(i) >= 0);
    }
...
async function rateLimited(userId, sbUrl, sbKey, token) {
  ...
  return rateLimitedScoped({ userId, scope: 'casey', limit: RATE_LIMIT,
