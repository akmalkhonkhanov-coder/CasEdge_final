...
function hintsBlock(step, attemptCount, asked, lang) {
  /* 23.08.2026. ПОДСКАЗКИ БРАЛИСЬ ТОЛЬКО ИЗ РУССКОЙ СТОРОНЫ. Цех кейсов сдал
     перевод целым объектом — `hints_en: {L1, L2}` рядом с `hints`, 319 единиц,
     638 строк. Движок читал `step.hints` напрямую, и перевод лёг бы мёртвым
     грузом: англоязычный кандидат получал бы русскую подсказку, а модель —
     русский текст в инструкции, стоящей последней перед ответом.
     Тот же класс, что скрытый экзибит утром: выборщик стоит не во всех точках
     чтения. Здесь он теперь стоит. */
