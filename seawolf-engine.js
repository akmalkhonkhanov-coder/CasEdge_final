  ...
  reveal(lang) {
    ...
    return this.game.sites.map((site, si) => {
      ...
      return {
        ...
        spread,
        /* 07.09.2026, dev. Поле НАЗЫВАЕТ свойство УЧАСТКА, а считало пул КАНДИДАТА:
           `vt.length === 0` значит «в моём пуле нет стопроцентного трио», а это и
           «потолка не было», и «я его выбросил на доборе» — ровно те два случая,
           ради различения которых делались круги 169 и 170. На площадке с потолком
           100, где кандидат потерял решение в раунде 3, поле говорило true.
           Вердикт считает правильно (`ceil` выше), врало только это поле; клиент его
           сегодня не печатает, поэтому до кандидата ложь не дошла. Но имя обещает
           факт об участке, и первый, кто его отрисует, напечатает «идеала не было»
           там, где идеал был. Считаем от участка, как и обещает имя. */
        ceilingBelow100: ceil.ceiling < 100,
