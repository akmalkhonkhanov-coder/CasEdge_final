...
#screen-casey { position:fixed; inset:0; z-index:50; height:100vh; height:100dvh; overflow:hidden; background:var(--surface-dark); display:none; flex-direction:column; }
...
.cy-bbl b { color:var(--on-dark,#faf9f5); } .cy-bbl code { background:rgba(255,255,255,.08); padding:1px 5px; border-radius:4px; font-size:.92em; }
/* exhibit card */
...
.cy-ex-title { font-size:15px; font-weight:700; color:var(--on-dark,#faf9f5); margin-bottom:12px; }
...
.cy-ex-warn { font-size:13px; color:#ef9a9a; background:rgba(232,124,124,.10); border:1px solid rgba(232,124,124,.35); border-radius:8px; padding:8px 11px; }
...
.cy-tbl tr.cy-tbl-total td { border-top:1.5px solid rgba(93,184,166,.4); font-weight:700; color:var(--on-dark,#faf9f5); }
...
.cy-numin, .cy-txtin { flex:1; background:var(--surface-dark-soft,#efe9dd); border:1.5px solid var(--sv-line,rgba(255,255,255,.14)); border-radius:10px; padding:12px 14px; color:var(--on-dark,#faf9f5); font-size:15px; font-family:inherit; }
...
.cy-fb.ok { background:rgba(95,191,107,.10); border:1px solid rgba(95,191,107,.35); color:#7fd68e; }
.cy-fb.no { background:rgba(232,124,124,.10); border:1px solid rgba(232,124,124,.35); color:#ef9a9a; }
.cy-fb b { color:var(--on-dark,#faf9f5); }
/* case picker */
...
.cy-pick-h h2 { font-size:24px; color:var(--on-dark,#faf9f5); margin:8px 0 6px; }
...
.cy-card .cy-cn { font-size:15px; font-weight:700; color:var(--on-dark,#faf9f5); }
...
.cy-recbtn.recording { background:#c94b4b; color:var(--on-dark,#faf9f5); animation:cyPulse 1.1s infinite; }
...
.cy-score { font-size:13px; font-weight:700; color:var(--on-dark,#faf9f5); margin:4px 0 12px; }
...
.cy-calc-head { display:flex; justify-content:space-between; align-items:center; color:var(--on-dark,#faf9f5); font-size:11px; font-weight:700; letter-spacing:.07em; text-transform:uppercase; margin-bottom:9px; }
...
.cy-calc-disp { width:100%; background:var(--surface-dark-soft,#efe9dd); border:1px solid var(--sv-line, rgba(31,41,55,.14)); border-radius:9px; color:var(--on-dark,#faf9f5); font-size:22px; text-align:right; padding:9px 11px; margin-bottom:10px; font-variant-numeric:tabular-nums; box-sizing:border-box; }
.cy-calc-keys { display:grid; grid-template-columns:repeat(4,1fr); gap:6px; }
.cy-calc-keys button { padding:12px 0; border:1px solid var(--sv-line-soft, rgba(31,41,55,.08)); border-radius:9px; background:var(--surface-dark-soft,#efe9dd); color:var(--on-dark,#faf9f5); font-size:15px; font-weight:600; cursor:pointer; }
...
(function () {
  ...
  function finish(){
    ...
    try { if (typeof recordSession === 'function') recordSession('game', 'BCG Chat Game', maxScore > 0 ? Math.round((S.score / maxScore) * 100) / 10 : undefined); } catch (e) {}
  }
